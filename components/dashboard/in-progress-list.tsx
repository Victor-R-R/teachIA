...
const DOMAIN_ACCENT: Record<string, string> = {
  langue: 'border-l-violet-400',
  civi_espagne: 'border-l-blue-400',
  civi_latam: 'border-l-emerald-400',
  didactique: 'border-l-amber-400',
}
      ...
      <div className="grid gap-2">
        {exercises.map(ex => (
          <Link key={ex.id} href={`/exercices/${ex.id}`}>
            <Card className={`bg-white border-slate-200 border-l-4 hover:shadow-sm transition-all cursor-pointer overflow-hidden ${DOMAIN_ACCENT[ex.domain] ?? 'border-l-slate-300'}`}>
              <CardContent className="p-3 flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-900 line-clamp-1 mb-1">{ex.question}</p>
        ...
        {simulations.map(sim => (
          <Link key={sim.id} href={`/simulacros/${sim.id}`}>
            <Card className="bg-white border-slate-200 border-l-4 border-l-rose-400 hover:shadow-sm transition-all cursor-pointer overflow-hidden">
              <CardContent className="p-3 flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
